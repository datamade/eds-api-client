...
def test_register_user(api_client, app_creds, new_user):
    response = api_client.register_user(new_user, app_creds['username'], app_creds['password'])
...
def test_invalid_register_user(api_client, app_creds, invalid_user):
    response = api_client.register_user(invalid_user, app_creds['username'], app_creds['password'])
    assert response.status_code == 400
