...
def web_client():
    return EDSWebClient(
        os.environ["TEST_VENDOR_USERNAME"],
        os.environ["TEST_VENDOR_PASSWORD"],
        retry_attempts=0,
    )
...
def new_user():
    ...
    new_user["user"].update({"userId": applicant[0], "email": applicant[1]})

    new_user["company"].update({"ssn": applicant[2]})
    return new_user
